                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    image_double image;
    ntuple_list out;
    unsigned int X;//* x image size /
    unsigned int Y;//* y image size /
    unsigned int verbose=1;//* whether the whole input and output should also be dumped /

    FILE* fout = fopen("lsdmexlog.txt", "wt");
  ...
  //* LSD parameters end*/
    ...
    if (!(mxIsDouble(prhs[0]))) {
      mexErrMsgTxt("Input image must be of type double.");
    }
    ...
    if (nrhs>2){
        sigma_scale=mxGetScalar(prhs[2]);
    }
    if (nrhs>3){
        quant=mxGetScalar(prhs[3]);
    }
    if (nrhs>4){
        ang_th=mxGetScalar(prhs[4]);
    }
    if (nrhs>5){
        eps=mxGetScalar(prhs[5]);
    }
    if (nrhs>6){
        density_th=mxGetScalar(prhs[6]);
    }
    if (nrhs>7){
        n_bins=mxGetScalar(prhs[7]);
    }
    if (nrhs>8){
        max_grad=mxGetScalar(prhs[8]);
    }
    if (nrhs > 9) {
    	mexErrMsgTxt("Too many input arguments.");
    } 
  
    fprintf(fout,"Input arguments verified\n");
    ...
    //* check whether the input is a gray-scale image/
    number_of_dims=mxGetNumberOfDimensions(prhs[0]);
    if (number_of_dims!=2) {
      mexErrMsgTxt("Input must be a gray-scale image");
    }

    //* Get the image size in the input argument. /
    dim_array=mxGetDimensions(prhs[0]);
   X= dim_array[0];  
   Y= dim_array[1];  
    fprintf(fout,"Gray scale image of size : %d x %d\n",X,Y);

//* code for calling the LSD routine here */    


  //* transfer data to the image buffer for LSD /
  image = new_image_double(X,Y);
  fprintf(fout,"\n--------\nInput data being written to image buffer \n");
  for(j=0;j<elements;j++){
      image->data[j] = pr[j];
	if (verbose){
	    fprintf(fout,"%f\t",image->data[j]);
	}
  }
  ...
  out = LineSegmentDetection( image, scale, sigma_scale, quant, ang_th, eps,
                               density_th, n_bins, max_grad, NULL );
  fprintf(fout,"LSD has done it's thing\n");

  plhs[0]=mxCreateDoubleMatrix(out->dim,out->size,mxREAL);
  pind=mxGetPr(plhs[0]);

  //* print output /
  fprintf(fout,"\n--------\nResult being written to output buffer \n");
  fprintf(fout,"n tuple of size : %d x %d\n",out->size,out->dim);
  for(i=0;i<(out->size)*(out->dim);i++)
  {
    pind[i]=out->values[ i ];
    if (verbose){
        fprintf(fout,"%f\t",pind[i]);
    }
  }
  if (verbose){
	fprintf(fout,"\n------\n\n");
